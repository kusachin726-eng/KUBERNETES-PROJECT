...
class DamageReportController {
    ...
    async uploadDamageImages(req, res, next) {
        const folder = "customer/damage-reports";
        let files = [];
        ...
        // Extract file names (or URLs)
        const uploadedFiles = uploadResults.map(result => result.fileName);
