...
const schema = {
  ...
  bookingAddressSchema: Joi.object({
    ...
    alternate_mobile: Joi.string().pattern(/^[6-9]\d{9}$/).optional().allow(null, "").messages({ 'string.pattern.base': 'Alternate mobile number must be a valid 10-digit Indian number' }),
    flat_house_number: Joi.string().trim().max(100).required(),
    street_building: Joi.string().trim().max(150).required().allow(null, ""),
  ...
  cancelBookingSchema: Joi.object({
    bookingId: Joi.number().integer().positive().required(),
    cancelReason: Joi.string().trim().max(500).optional().allow(null, "")
  })
};
